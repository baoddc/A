...
function openDeleteDataModal() {
  ...
  const perms = (typeof getUserPermissions === 'function') ? getUserPermissions('tole') : { canDelete: true, isAdmin: true };
  ...
  try {
    ...
    if (e.target && e.target.id === 'addDataForm') {
      ...
      if (insertedData) {
        ...
        // Update local SWR cache for tole-xuat
    ...
    // ===== EDIT DATA =====
      ...
      if (updatedData && updatedData[0]) {
        ...
        broadcastToleEvent('TOLE_XUAT_UPDATE', {
...
document.addEventListener('click', async (e) => {
  if (e.target && e.target.id === 'btnConfirmDelete') {
    ...
    try {
      ...
      broadcastToleEvent('TOLE_XUAT_DELETE', {
