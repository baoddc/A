/* =============================================================================
   XG-XUAT - SUPABASE VERSION
   Quản lý dữ liệu xuất xà gồ từ Supabase
   Tên bảng: xg-xuat
...
// Tên bảng Supabase
const TABLE_NAME = 'xg-xuat';
const TON_TABLE_NAME = 'xg-ton'; // Bảng tồn kho

// Header cột (theo thứ tự trong bảng xg-xuat)
...
window.addEventListener('load', () => {
  ...
  if (xgChannel) {
    xgChannel.onmessage = (e) => {
      const { type, payload } = e.data || {};
      if (type === 'XG_XUAT_INSERT' && payload && Array.isArray(payload.cuonIds)) {
...
async function openInventoryModal(target, maVatTu = '', batch = '') {
  ...
  try {
    ...
    // Lấy danh sách Cuộn ID đã xuất mới nhất từ Database xg-xuat
    ...
    try {
      ...
      if (!xuatErr && Array.isArray(xuatData)) {
        ...
        throw xuatErr || new Error('Không thể tải danh sách xg-xuat');
      }
    } catch (e) {
    ...
    // Tính toán dữ liệu tồn: Lọc các cuộn trong xg-nhap chưa bị xuất
  ...
  if (e.target && e.target.id === 'btnConfirmInventorySelection') {
    ...
    if (target === 'add_item' || target === 'add') {
      ...
      selectedCheckboxes.forEach(cb => {
        ...
        const tonKg = parseNumericInput(cb.dataset.tonKg) || 0;
        if (!existingCuonIds.has(cuonId.toLowerCase())) {
          addEditRollRow(cuonId, String(tonKg));
        }
      });
    }
...
function addRollRow(maVatTu = '', cuonId = '', kgValue = '') {
  rollCount++;
...
function addEditRollRow(cuonId = '', kgValue = '') {
  editRollCount++;
...
document.addEventListener('submit', async (e) => {
  try {
    ...
    if (e.target && e.target.id === 'addDataForm') {
      ...
      try {
        const { data, error } = await supabase.rpc('xuat_xg_atomic', {
        ...
        console.error('Lỗi giao dịch xuất kho RPC xuat_xg_atomic:', rpcErr);
    ...
    // ===== EDIT DATA =====
      ...
      document.querySelectorAll('#editRollsTableBody tr').forEach(row => {
        ...
        if (kgInput && kgInput.value) {
          const parsed = parseNumericInput(kgInput.value);
          if (parsed !== null && parsed > 0) rollKgValues.push(parsed);
        }
      });
